...
apply_cish_plot_style!(size = (900, 700))

const TUMOR_COLORS = CISH_FIGURE_COLORS
...
function main(; verbose=true, nTotal=200, nRefine=10)
    ...
    vars_fit_name_patient = ["dL","kL","pL","muL","f_dose","kappa"]
    ...
    for i in 1:2
        ...
        fig = plot(layout=(2,2), size=(900,700), legend = :best)
        ...
        for k in 1:(size(tumors_size,2) ÷ 2)
            ...
            plot!(fig[1], sol.x, sol.y[1, :],
                  label = "Tumor $k", lw = 2.4, color = col)
            
            scatter!(fig[1], tumor_size[:,1], tumor_size[:,2],
                     label = "Data $k", markersize = 3.5,
                     markercolor = col, markerstrokecolor = :white,
                     markerstrokewidth = 0.6)

            # Panel 2: WildType (N)
            plot!(fig[2], sol.x, sol.y[3, :], label="WildType $k", lw=2.0, color = col)

            # Panel 3: CISHKO (L)
            plot!(fig[3], sol.x, sol.y[2, :], label="CISHKO $k", lw=2.0, color = col)
        ...
        if size(cish_ratio,1) > 0
            scatter!(fig[4], cish_ratio[:,1], cish_ratio[:,2],
                     markersize=3.5, label="Ratio data",
        ...
        save_png_pdf(fig, "Patient_$(id_patient[i])_fit.png")
