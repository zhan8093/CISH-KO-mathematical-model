...
# ============================================================
# Leave-one-therapy-out cross-validation for ALL 6 therapies.
# For each therapy i in 1..6: fit on the other 5, then predict i
# and compare against the held-out data. Produces one figure per
# fold plus a combined 2x3 panel and a summary CSV.
# ============================================================
...
init_vals = [7.5, 42.0,
             0.157606, 1.0,
             0.741223, 0.189675, 1e-4, 1.2e-4, 4e-4,
             0.145898, 0.03, 0.417132, 0.05,
             0.01, 1e3,
             0.7, 0.151213, 50, 0.01, 0.0446]
...
default(fontfamily="Helvetica", framestyle=:box, grid=false,
        guidefontsize=11, tickfontsize=10, legendfontsize=8, titlefontsize=11)

fold_paras = Vector{Any}(undef, 6)
fold_rmse  = zeros(6)
fold_r2    = zeros(6)
subplots   = Vector{Any}(undef, 6)
...
for val_idx in 1:6
    ...
    fold_paras[val_idx] = paras_opt

    d_val = DATASETS[][val_idx]
    sol_val = solve_model(paras_opt, u0, conds[val_idx]; saveat=d_val.t)
    ...
    sol_full = solve_model(paras_opt, u0, conds[val_idx])
...
# ============================================================
# Combined 2x3 panel figure
# ============================================================
combined = plot(subplots..., layout=(2,3), size=(1500, 900),
                plot_title="Leave-one-therapy-out cross-validation",
...
for row in eachrow(summary_rows)
    @printf("  Fold %d  %-18s  RMSE=%8.3f  R²=%7.4f\n",
            row.Fold, row.Validation, row.RMSE, row.R2)
end
@printf("  Mean RMSE=%.3f  Mean R²=%.4f\n", mean(fold_rmse), mean(fold_r2))

println("\nSaved:")
println("  crossval_validation_1..6.png  (one per fold)")
println("  crossval_all_folds.png        (2x3 panel)")
