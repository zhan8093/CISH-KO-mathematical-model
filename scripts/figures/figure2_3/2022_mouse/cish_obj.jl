...
function init_datasets()
    files = [
        ...
        ("wild type isotype control.xlsx", (0.0, 0.5, 0.0), 2.0),
        ("wild type antiPD1.xlsx",         (0.0, 0.5, 5.0), 3.0),
        ("CISH KO isotype control.xlsx",   (0.5, 0.0, 0.0), 5.0),
        ("CISH KO antiPD1.xlsx",           (0.5, 0.0, 5.0), 5.0),
    ]
    ds = []
    for (f, cond, w) in files
        t, v = load_data(f)
        scale = max(maximum(v), 10.0)
        push!(ds, (t=t, v=v, cond=cond, w=w, scale=scale))
    end
    DATASETS[] = ds
    return ds
end
